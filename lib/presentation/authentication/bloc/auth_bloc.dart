      ...
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          emit(AuthAuthenticated(
              user, 'Account created succesfully!!', username));
          ...
          debugPrint("${response.data['message'] ?? 'Registration failed'}");
        }
      } on DioException catch (e) {
        emit(AuthError(e.response?.data['message'] ??
            'Network/Server error during register'));
        debugPrint(
            "${e.response?.data['message'] ?? 'Network/Server error during register'}");
      }
    } catch (e) {
  ...
  Future<void> loginAccount(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final user = UserModel.fromJson(response.data);
        final username = response.data['data']['user']['name'];
        emit(AuthAuthenticated(user, 'Login succesfully!!', username));
